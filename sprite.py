...
for o in range(0, output):
    ...
    # Process images in row-major order (across rows first, then down)
    for y in range(0, rows):          # Changed from columns to rows
        for x in range(0, columns):   # Changed from rows to columns
            if current_image >= len(filelist):
                break  # Safety check to avoid index errors

            try:
                with Image.open(filelist[current_image]) as img:
                    # Position calculation matches the loop order
                    new_image.paste(img, (x*width, y*height))
                current_image += 1
            except Exception as e:
                print(f"Error processing {filelist[current_image]}: {e}")
                current_image += 1
                continue
